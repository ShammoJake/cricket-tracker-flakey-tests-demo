...
public class LiveScoreIngestTest {
    ...
    /**
     * Settle time for the FlakeSync target below. FlakeSync always invokes a single test
     * method, which leaves the pipeline far colder than the suite: tools/Calibrate reports
     * ~30ms for the first ingest and ~5ms for the second, against a warm band of 1.3-2.5ms.
     * With SETTLE_MILLIS the delay-0 baseline therefore failed 5/5 in isolation, and
     * FlakeSync needs that baseline to pass before an injected delay can mean anything.
     * Calibrated against single-method execution; see Running_FlakeSync.md.
     */
    private static final long ISOLATED_SETTLE_MILLIS = 100L;
    ...
    public void scorecardReflectsAllBallsAfterIngest() throws InterruptedException {
        // Barrier generated by FlakeSync (patch/LiveScoreIngestTest.java.patch), with the
        // threshold corrected from 240 to DELIVERIES. FlakeSync measured 240 during
        // barrierpointsearch -- 120 warmup tasks from primePipeline() plus 120 from this
        // test -- but the patch it emitted resets the counter *after* the warmup has run,
        // so only this test's 120 tasks can ever be counted and the original barrier spun
        // forever. See FlakeSync_results.md.
        com.cricket.live.WorkerPool.resetExecutions();
